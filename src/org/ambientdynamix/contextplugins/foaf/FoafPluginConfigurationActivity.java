...
public class FoafPluginConfigurationActivity extends Activity implements IContextPluginConfigurationViewFactory
{
	...
	public View initializeView(Context context, final ContextPluginRuntime arg1, int arg2) throws Exception 
	{
        ...
        if(settings!=null)
        {
            	...
            	String u = settings.get(Constants.FOAFURL);
            	if(u!=null)
            	{
            		username.setText(u);
            	}
            ...
            {
            	settings = FoafPluginRuntime.settings;
                if(settings!=null)
                {
                	Log.d(TAG, "got Settings");
                	String u = settings.get(Constants.FOAFURL);
        ...
        b2.setOnClickListener(new View.OnClickListener() 
        {
            public void onClick(View v)
            {
            	String x = username.getEditableText().toString();
            	 ContextPluginSettings settings = arg1.getPluginFacade().getContextPluginSettings(arg1.getSessionId());
            	settings.put(Constants.FOAFURL, x);
            	//TODO: finish()
